    ...
    this.messageHandler = function(message) {
        switch (message.type) {
            ...
            case MSG_GAME_FOUND:
                ...
                break;
            case MSG_EXIT_FROM_MENU:
                _oThis._onExitFromMenu(message);
                break;
            case MSG_SET_WINNER:
                _oThis._onSetWinner(message);
                break;
            case MSG_CHANGE_TURN:
                _oThis._onChangeTurn(message);
                break;
            case MSG_REMATCH_PANEL:
                _oThis._onRematchPanel(message);
                break;

            case MSG_PLAYER_DISCONNECTED_WHILE_ENTERING_IN_GAME:
                _oThis._onPlayerDisconnectionWhileEnteringInGame(message);
                break;
            ...
            case MSG_REMATCH_ANSWER_RESULTS:
                ...
                break;
            case MSG_STRIKER_KICKS_RESULTS:
                _oThis._onStrikerKicksResults(message);
                break;
            case MSG_GOALKEEPER_SAVES_RESULTS:
                _oThis._onGoalkeeperSavesResults(message);
                break;
            case MSG_NEXT_PLAYER_TO_PICK_TEAM:
                _oThis._onNextPlayerToPickTeam(message);
                break;
            case MSG_ALL_TEAM_SELECTED:
                _oThis._onAllTeamSelected(message);
                break;
            case MSG_RECEIVE_QUIZ:
                _oThis._onQuizReceived(message);
                break;
            case MSG_PLAYER_ANSWER:
                _oThis._onPlayerAnswerChosen(message);
                break;
            case MSG_PLAYER_ACCEPTED_A_REMATCH_NOTIFY:
                _oThis._onNotifyPlayerAcceptedRematch(message);
                break;
        }
    };
