function CScenarioStriker() {
        ...
        for (var i = 0; i < NUM_AREA_GOAL.h; i++) {
            for (var j = 0; j < NUM_AREA_GOAL.w; j++) {
                s_oScenario.createAreaGoal({
                    ...
                    z: fZ
                }, AREA_GOAL_PROPERTIES, COLOR_AREA_GOAL[iID], AREAS_INFO[iID]);
    ...
    this._createFieldBody = function() {
        ...
        if (SHOW_3D_RENDER) {
            var oFieldMaterial = new THREE.MeshPhongMaterial({
                color: 0x588e30,
                specular: 0x111111,
                shininess: 10
            }); //0x588e30
    ...
    this._createGoal = function() {
        ...
        _oGoalBodyPoleLeftRight = new CANNON.Body({
            mass: 0
        });
        ...
        if (SHOW_3D_RENDER) {
            var oGoalMaterial = new THREE.MeshPhongMaterial({
                color: 0xffffff,
                specular: 0x111111,
                shininess: 50
            });
    ...
    this.createBackGoalWall = function() {
        ...
        _oGoalBodyWall = new CANNON.Body({
            mass: 0,
    ...
    this.createAreaGoal = function(oPos, oProperties, oColor, oInfo) {
        ...
        var oGoalBodyLine = new CANNON.Body({
            mass: 0,
            userData: oInfo
        });
        ...
        if (SHOW_3D_RENDER) {
            var oMaterial = new THREE.MeshPhongMaterial({
                color: oColor,
                specular: 0x111111,
                shininess: 70
            });
    ...
    this._createBallBody = function() {
        ...
        if (SHOW_3D_RENDER) {
            var oBallMaterial = new THREE.MeshPhongMaterial({
                color: 0xffffff,
                specular: 0x111111,
                shininess: 70
            });
